...
Analytics.prototype.add = function(integration, count){
  ...
  this._integrations[integration.name + '%%' + count] = integration;
  return this;
};
...
GA.prototype.initialize = function() {
  ...
  this.options.trackerName = getUATrackerName(this.options.trackingId, GA.defaults.trackerName);

  var opts = this.options;
  var ga_s = document.location.search;
  var re = new RegExp('[\?&#]'+ GA.defaults.cookieName +'=([^\?&#]*)');
  var xparam = ga_s.match(re) != null ? ga_s.match(re)[1] : undefined;
  ...
  // display advertising
  if (opts.doubleClick) {
    window.ga(opts.trackerName + '.require', 'displayfeatures');
  }

  // send global id
  if (opts.sendUserId && user.id()) {
    window.ga(opts.trackerName + '.set', 'userId', user.id());
  }

  // anonymize after initializing, otherwise a warning is shown
  // in google analytics debugger
  if (opts.anonymizeIp) window.ga(opts.trackerName + '.set', 'anonymizeIp', true);
...
GA.prototype.page = function(page) {
  ...
  // set
  window.ga(opts.trackerName + '.set', { page: pagePath, title: pageTitle });


  // send
  window.ga(opts.trackerName + '.send', 'pageview', pageview);
...
GA.prototype.identify = function(identify) {
  ...
  if (opts.sendUserId && identify.userId()) {
    window.ga(opts.trackerName + '.set', 'userId', identify.userId());
  }

  // Set dimensions
  var custom = metrics(user.traits(), opts);
  if (len(custom)) window.ga(opts.trackerName + '.set', custom);
};
...
GA.prototype.completedOrder = function(track) {
  ...
  // require ecommerce
  if (!this.ecommerce) {
    window.ga(opts.trackerName + '.require', 'ecommerce');
    this.ecommerce = true;
  }

  // add transaction
  window.ga(opts.trackerName + '.ecommerce:addTransaction', {
...
var SimpleImagePixel = module.exports = integration('Simple Image Pixel')
  .option('pixelId', '')
  .tag('<img src="{{ pixelId }}"/>')
  .mapping('events');
...
SimpleImagePixel.prototype.completedOrder = function(track) {
  var events = this.events('event_order_confirmation');

  for (var i = 0; i < events.length; i += 1) {
    var pixelId = events[i];

    pixelId = pixelId.replace(/orderid/gi, track.orderId());
    pixelId = pixelId.replace(/ordertotal/gi, track.total());
    pixelId = pixelId.replace(/ordercurrency/gi, track.currency());
    this.load({ pixelId: pixelId });
  }
};
...
var AdrollIPixel = module.exports = integration('Adroll iPixel')
  .option('pixelId', '')
  .tag('<img src="{{ pixelId }}"/>')
  .mapping('events');
